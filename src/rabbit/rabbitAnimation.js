...
import {TweenMax, Power4, Back, Power2, Power1 } from 'gsap';
...
function createFloor() {
  ...
  let floorShadow = new THREE.Mesh(new THREE.SphereGeometry(floorRadius, 50, 50), new THREE.MeshPhongMaterial({
    color: 0x7abf8e,
    specular:0x000000,
    shininess:1,
    transparent:true,
  ...
  let floorGrass = new THREE.Mesh(new THREE.SphereGeometry(floorRadius-.5, 50, 50), new THREE.MeshBasicMaterial({
    color: 0x7abf8e
  }));
...
function gameOver(){
  document.getElementById("gameoverInstructions").style.opacity = 1;
  gameStatus = "gameOver";
  monster.sit();
  hero.hang();
...
function replay(){
  ...
  document.getElementById("gameoverInstructions").style.opacity = 0;
